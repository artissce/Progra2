...
void agregarPosicion(){
	...
	if(pos == 1){
		agregarCircularSimple(1);  //con esto hacemos que agregue al inicio
		getchar();
	}else {
		while (aux!=NULL){
			if(i == (pos-1)){
		...
		if(bandera){
			cout<<"La posicion no existe, el nodo se agregara al final de la lista"<<endl;
			agregarCircularSimple(2);
		}
	}
}

void agregarPosicionSim(){
	struct Empleado *aux = NULL;
	struct Empleado *aux2 = NULL;
	struct Empleado *freno = NULL;
	aux = freno =  primero; 
	bool bandera = true;
	int pos = 0;
	int i = 1;
...
void agregarPosicioDob(){
	...
	if(pos == 1){
		 agregarCircularDoble(1);  //con esto hacemos que agregue al inicio
		getchar();
	}else {
		while (aux!=NULL){
			if(i == (pos-1)){
